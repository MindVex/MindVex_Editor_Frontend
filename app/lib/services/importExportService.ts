...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          mindvex_user_profile: this._safeGetItem('mindvex_user_profile'),
          mindvex_settings: this._safeGetItem('mindvex_settings'),
          mindvex_profile: this._safeGetItem('mindvex_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('mindvex_viewed_features'),
          developer_mode: this._safeGetItem('mindvex_developer_mode'),
        ...
        ui: {
          // Tab configuration
          mindvex_tab_configuration: this._safeGetItem('mindvex_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('mindvex_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('mindvex_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          mindvex_read_logs: this._safeGetItem('mindvex_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('mindvex_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('mindvex_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.mindvex_tab_configuration) {
        try {
          this._safeSetItem('mindvex_tab_configuration', data.ui.mindvex_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'mindvex_acknowledged_debug_issues',
        'mindvex_acknowledged_connection_issue',
        'error_logs',
        'mindvex_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('mindvex_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {

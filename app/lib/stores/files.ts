...
export class FilesStore {
  ...
  constructor(webcontainerPromise: Promise<WebContainer>) {
    ...
    try {
      if (typeof localStorage !== 'undefined') {
        const deletedPathsJson = localStorage.getItem('mindvex-deleted-paths');
  ...
  async createFile(filePath: string, content: string | Uint8Array = '') {
    ...
    try {
      ...
      console.error('Error creating file at path:', filePath);
      console.error('WebContainer workdir was:', webcontainer.workdir);
      throw error;
    }
  }

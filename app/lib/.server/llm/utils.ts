...
export function simplifyBoltActions(input: string): string {
  // Using regex to match mindvexAction tags that have type="file"
  const regex = /(<mindvexAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/mindvexAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<mindvexAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</mindvexAction>`;
    });

  return `<mindvexArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</mindvexArtifact>`;
}

    ...
    const components = useMemo(() => {
      return {
        div: ({ className, children, node, ...props }) => {
          ...
          if (className?.includes('__mindvexArtifact__')) {
          ...
          if (className?.includes('__mindvexSelectedElement__')) {
            const messageId = node?.properties.dataMessageId as string;
            ...
            return (
              <div className="bg-mindvex-elements-background-depth-3 border border-mindvex-elements-borderColor rounded-lg p-3 my-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-mono bg-mindvex-elements-background-depth-2 px-2 py-1 rounded text-mindvex-elements-textTer">
                    {elementData?.tagName}
                  </span>
                  {elementData?.className && (
                    <span className="text-xs text-mindvex-elements-textSecondary">.{elementData.className}</span>
                  )}
                </div>
                <code className="block text-sm !text-mindvex-elements-textSecondary !bg-mindvex-elements-background-depth-2 border border-mindvex-elements-borderColor p-2 rounded">
          ...
          if (className?.includes('__mindvexThought__')) {
            return <ThoughtBox title="Thought process">{children}</ThoughtBox>;
          }

          if (className?.includes('__mindvexQuickAction__') || dataProps?.dataMindvexQuickAction) {
        ...
        button: ({ node, children, ...props }) => {
          ...
          if (
            dataProps?.class?.toString().includes('__mindvexQuickAction__') &&
            dataProps?.dataMindvexQuickAction === 'true'
          ) {
            ...
            return (
              <button
                className="rounded-md justify-center px-3 py-1.5 text-xs bg-mindvex-elements-item-backgroundAccent text-mindvex-elements-item-contentAccent opacity-90 hover:opacity-100 flex items-center gap-2 cursor-pointer"
                data-type={type}
 ...
 * const input = "```xml\n<div class='__mindvexArtifact__'></div>\n```";
 * stripCodeFenceFromArtifact(input);
 * // Returns: "\n<div class='__mindvexArtifact__'></div>\n"
 *
 * @remarks
 * - Only removes code fences that directly wrap an artifact (marked with __mindvexArtifact__ class)

      ...
      {shouldShowButtons && (
        <div className="flex border border-mindvex-elements-borderColor rounded-md overflow-hidden text-sm">
          <button
            ...
            className="rounded-l-md items-center justify-center [&:is(:disabled,.disabled)]:cursor-not-allowed [&:is(:disabled,.disabled)]:opacity-60 px-3 py-1.5 text-xs bg-accent-500 text-white hover:text-mindvex-elements-item-contentAccent [&:not(:disabled,.disabled)]:hover:bg-mindvex-elements-button-primary-backgroundHover outline-accent-500 flex gap-1.5"
            title="Report Bug"
          >
            <div className="i-ph:bug" />
            <span>Report Bug</span>
          </button>
          <div className="w-px bg-mindvex-elements-borderColor" />
          <button
            onClick={async () => {
              try {

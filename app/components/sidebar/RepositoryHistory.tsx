...
import { importGitRepoToWorkbench, importFolderToWorkbench, loadStoredFolderToWorkbench, loadStoredGitRepoToWorkbench } from '~/utils/workbenchImport';
...
export function RepositoryHistory({
  ...
  const allRepositories = Object.values(repositoryHistory).filter(repo => 
  ...
  const handleOpenInWorkbench = async (repo: RepositoryHistoryItem) => {
    try {
      ...
      if (repo.url.startsWith('folder://')) {
        ...
        const addToExisting = window.confirm(`Do you want to add '${repo.name}' to the existing workspace?\n\nClick 'OK' to add to existing workspace, 'Cancel' to create a new workspace (replacing current content)`);
        
        try {
          // Attempt to load the stored folder
          const success = await loadStoredFolderToWorkbench(folderName, addToExisting);
          if (success) {
            toast.success(`Folder '${repo.name}' loaded in workbench ${addToExisting ? 'with existing content' : '(workspace cleared)'}`);
          }
        } catch (error) {
          console.error('Error loading stored folder:', error);
          // If loading from storage fails, prompt for re-upload
          const shouldReupload = window.confirm(`Failed to load stored folder '${repo.name}'. Would you like to select the folder again to reload it?`);
        ...
        const repoName = repo.url.split('/').pop()?.replace(/\.git$/, '') || repo.name;
        ...
        try {
          ...
          if (success) {
            toast.success(`Repository '${repo.name}' loaded in workbench ${addToExisting ? 'with existing content' : '(workspace cleared)'}`);
          }
        } catch (error) {
          console.error('Error loading stored repository:', error);
          // If loading from storage fails, ask user if they want to clone again
          const shouldClone = window.confirm(`Failed to load stored repository '${repo.name}'. Would you like to clone it again?`);

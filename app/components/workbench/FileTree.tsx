      ...
      input.onchange = async (e) => {
        ...
        if (files.length > 0) {
          try {
            ...
            toast.error('Error importing folder');
          }
        }
      };
    ...
    return (
      <div className={classNames('text-sm', className, 'overflow-y-auto modern-scrollbar')}>
        {filteredFileList.length > 0 ? (
          ...
          <div className="px-2 py-4 text-center text-mindvex-elements-textSecondary text-sm">
            <DropdownMenu.Root>
              <DropdownMenu.Trigger asChild>
                <div className="cursor-pointer hover:text-mindvex-elements-textPrimary transition-theme">
                  <div className="i-ph:folder-open text-lg mb-2" />
                  <p>No files available</p>
                  <p className="text-xs mt-1 opacity-75">Import a folder or create new files</p>
                </div>
              ...
              <DropdownMenu.Portal>
                ...
                >
                  ...
                  >
                    <span className="i-ph:folder-open" />
                    Open Folder
                  </DropdownMenu.Item>
                </DropdownMenu.Content>
              </DropdownMenu.Portal>
...
function ContextMenuItem({ onSelect, children }: { onSelect?: () => void; children: ReactNode }) {
  return (
    <ContextMenu.Item
      onSelect={onSelect}
      className="flex items-center gap-2 px-2 py-1.5 outline-0 text-sm text-mindvex-elements-textPrimary cursor-pointer ws-nowrap text-mindvex-elements-item-contentDefault hover:text-mindvex-elements-item-contentActive hover:bg-mindvex-elements-item-backgroundActive rounded-md"
    >
...
function InlineInput({ depth, placeholder, initialValue = '', onSubmit, onCancel }: InlineInputProps) {
  ...
  return (
    <div
      className="flex items-center w-full px-2 bg-mindvex-elements-background-depth-4 border border-mindvex-elements-item-contentAccent py-0.5 text-mindvex-elements-textPrimary"
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
    >
      <div className="scale-120 shrink-0 i-ph:file-plus text-mindvex-elements-textTertiary" />
      <input
        ref={inputRef}
        type="text"
        className="ml-2 flex-1 bg-transparent border-none outline-none py-0.5 text-sm text-mindvex-elements-textPrimary placeholder:text-mindvex-elements-textTertiary min-w-0"
    ...
    <>
      <ContextMenu.Root>
        <ContextMenu.Trigger>
          <div
            ...
            className={classNames('relative', {
              'bg-mindvex-elements-background-depth-2 border border-dashed border-mindvex-elements-item-contentAccent rounded-md':
                isDragging,
            })}
          >
            {children}
        ...
        <ContextMenu.Portal>
          <ContextMenu.Content
            style={{ zIndex: 998 }}
            className="border border-mindvex-elements-borderColor rounded-md z-context-menu bg-mindvex-elements-background-depth-1 dark:bg-mindvex-elements-background-depth-2 data-[state=open]:animate-in animate-duration-100 data-[state=open]:fade-in-0 data-[state=open]:zoom-in-98 w-56"
          >
            <ContextMenu.Group className="p-1 border-b-px border-solid border-mindvex-elements-borderColor">
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-mindvex-elements-borderColor">
              {!isFolder ? (
                <>
    ...
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={folder.fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent text-mindvex-elements-item-contentDefault hover:text-mindvex-elements-item-contentActive hover:bg-mindvex-elements-item-backgroundActive':
            !selected,
          'bg-mindvex-elements-item-backgroundAccent text-mindvex-elements-item-contentAccent': selected,
        })}
        depth={folder.depth}
...
function File({
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent hover:bg-mindvex-elements-item-backgroundActive text-mindvex-elements-item-contentDefault':
            !selected,
          'bg-mindvex-elements-item-backgroundAccent text-mindvex-elements-item-contentAccent': selected,
        })}
        depth={depth}
        iconClasses={classNames('i-ph:file-duotone scale-98', {
          'group-hover:text-mindvex-elements-item-contentActive': !selected,
        })}
        onClick={onClick}
      >
        <div
...
function buildFileList(
  ...
  for (const [filePath, dirent] of Object.entries(files)) {
    ...
    // Only process files that are within the rootFolder
    if (!normalizedFilePath.startsWith(rootFolder)) {
      continue;
    }
    ...
    while (i < segments.length) {
      ...
      // Skip if not under rootFolder or if it's the root folder itself and hideRoot is true

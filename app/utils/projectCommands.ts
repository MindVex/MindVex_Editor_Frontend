...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<mindvexAction type="shell">${commands.setupCommand}</mindvexAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<mindvexAction type="start">${commands.startCommand}</mindvexAction>
`;
  }
...
<mindvexArtifact id="project-setup" title="Project Setup">
${commandString}
</mindvexArtifact>`,
    id: generateId(),
    createdAt: new Date(),
...
export function escapeMindvexArtifactTags(input: string) {
  // Regular expression to match mindvexArtifact tags and their content
  const regex = /(<mindvexArtifact[^>]*>)([\s\S]*?)(<\/mindvexArtifact>)/g;
...
export function escapeMindvexActionTags(input: string) {
  // Regular expression to match mindvexArtifact tags and their content
  const regex = /(<mindvexAction[^>]*>)([\s\S]*?)(<\/mindvexAction>)/g;
...
export function escapeMindvexTags(input: string) {
  return escapeMindvexArtifactTags(escapeMindvexActionTags(input));
}

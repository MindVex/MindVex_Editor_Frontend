...
const bugReportSchema = z.object({
  ...
  environmentInfo: z
    .object({
      ...
      mindvexVersion: z.string().optional(),
...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.mindvexVersion) {
      body += `- mindvex: ${data.environmentInfo.mindvexVersion}\n`;
    }
  ...
  body += '---\n*Submitted via MindVex bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'mindvex-ai/mindvex';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'mindvex-bug-reporter',
    });
